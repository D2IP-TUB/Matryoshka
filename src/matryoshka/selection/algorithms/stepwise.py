...
from matryoshka.selection.sketch_processing import SketchProcessor
    ...
    def run(self, model: FeatureSelectionModel, task: str, joint_tuples, **kwargs):
        ...
        match task:
            case 'regression':
                ...
                # `_prepare_split_inputs` also returns per-column header names
                # (used elsewhere for sibling dedup); stepwise does not need
                # them, so drop the extra column.
                split_sums, split_diags, split_feature_indices, *_ = zip(*splits)
