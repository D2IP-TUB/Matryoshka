...
# Proxy-model scoring criteria implemented by
# ``matryoshka.selection.base.model.FeatureSelectionModel.score`` (regression)
# and ``matryoshka.selection.models.ClassificationCholesky`` (classification).
# Keep in sync with those two dispatch tables.
REGRESSION_METRICS = ['mse', 'rmse', 'gcv_rmse', 'conditional_gcv', 'r2', 'adj_r2']
CLASSIFICATION_METRICS = [
    'average_mahalanobis', 'average_bhattacharyya', 'robust_moment',
    'conditional_mahalanobis',
]
...
class DiscoveryConfig:
    ...
    _task_rules: Dict[str, Dict] = field(default_factory=lambda: {
        ...
        "regression": {
            ...
            "ranking_strategy_rules": {
                ...
                "correlation": {"allowed_strategy": [None, "BackwardElimination", "LassoFeatureSelector"]},
                "eta": {"impossible": True}
            }
        },
        "classification": {
            ...
            "ranking_strategy_rules": {
                "passthrough": {"allowed_strategy": [None, "LassoFeatureSelector", "IncrementalSelection", "ForwardSelection"]},
                "joinability": {"allowed_strategy": ["IncrementalSelection"]},
                "eta": {"allowed_strategy": [None, "BackwardElimination", "LassoFeatureSelector"]},
    ...
    _strategy_rules: Dict[str, Dict] = field(default_factory=lambda: {
        ...
        "IncrementalSelection": {
            ...
            "required_params": {
                "metric": {"type": str, "allowed_values": ["mse", "r2", "adj_r2", "average_mahalanobis", "average_bhattacharyya", "robust_moment"]},
        ...
        "ForwardSelection": {
            ...
            "required_params": {
                "metric": {"type": str, "allowed_values": REGRESSION_METRICS + CLASSIFICATION_METRICS},
                "tol": {"type": float, "validator": lambda x: x > 0}
            },
            "optional_params": {
                "polynomial_features": {
                    "type": dict,
                    "validator": lambda x: (
                        isinstance(x.get("enabled", False), bool)
                        and isinstance(x.get("degree", 2), int) and x.get("degree", 2) >= 2
                        and isinstance(x.get("interactions", False), bool)
                    )
                },
                "dummify_features": {
                    "type": dict,
                    "validator": lambda x: (
                        isinstance(x.get("enabled", False), bool)
                        and isinstance(x.get("n_bins", 4), int)
                        and x.get("n_bins", 4) >= 2
                    )
                }
            }
        },
