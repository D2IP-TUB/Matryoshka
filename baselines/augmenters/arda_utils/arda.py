...
def select_arda_features_budget_join(
    ...
    # Anytime instrumentation. Cooperative deadline + per-RIFS-batch trajectory
    # emit. Each outer iteration appends to ``final_selected_features``, so the
    # checkpoint after every successful RIFS round captures the algorithm's
    # progress.
    from matryoshka.selection.anytime import BudgetClock, TrajectoryEmitter
    _clock = BudgetClock(budget_seconds).start()
    _emitter = (TrajectoryEmitter(trajectory_dir, algo='ARDA')
    ...
    while len(nodes) > 0:
        ...
        # Paper-era target handling: run the feature generator over the full
        # frame (target included) so string labels are ordinal-encoded and
        # NaN targets mean-imputed alongside the features. The interim
        # variant that passed the raw target through crashes select_features
        # on string labels and NaN targets, silently yielding empty plans.
        # Fall back to raw-target alignment only when the generator drops
        # the label column.
        ...
        if target_column_name in X.columns:
            ...
            y = joined_tables_batch[target_column_name].copy()
            y = y.loc[X.index]
        if X.empty:
            continue
        ...
        if _emitter is not None:
            _iter_idx += 1
            _emitter.emit(_iter_idx, _clock.elapsed_s, list(final_selected_features))
