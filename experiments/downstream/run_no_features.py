...
if __name__ == "__main__":
    ...
    #         X, query_col, target, nan_mask = preprocessor.run(binning=False)
    #         X = X.select([col for col in X.columns if query_col in col or target in col])
    #         X.write_csv(
    #             os.path.join(
    #                 base_tables_dir,
    #                 base_table_name,
    ...
    #         log_dir = os.path.join(script_dir, 'logs2', combination_hash)
    #         if not os.path.exists(os.path.join(script_dir, 'logs2', combination_hash)):
    #             os.makedirs(log_dir)
    ...
    experiment_dir = os.listdir('experiments/downstream/logs2/')
    trainers = {
    ...
    for trainer_name in trainers:
        ...
        for dir in experiment_dir:
            if not os.path.isdir(os.path.join('experiments/downstream/logs2/', dir)):
                continue
            log_path = os.path.join('experiments/downstream/logs2/', dir)
            print(dir)
        ...
        all_results.to_csv(f'experiments/downstream/logs2/{trainer_name}_results.csv')
