...
if __name__ == "__main__":
    ...
    split_index_choices = [1, 2, 3]
    for split_index in split_index_choices:
        for lake, config in config_df.group_by('lake'):
            ...
            for row in config.iter_rows():
                ...
                with open(base_table_splits_path, 'r') as f:
                    splits = json.load(f)
                features = splits[split_index]['features']
                preprocessor = PreProcessor(base_table_path, base_table_splits_path, split_index)
                ...
                combination_hash = f'{lake}_{base_table_name}_{split_index}'
                ...
                find_best_joins_kwargs = {'user_table_processed': X, 'top_k': 50, 'n_jobs': 64, 'config': config}
                ...
                try:
                    find_best_joins_kwargs['query_column_name'] = query_col
