...
if __name__ == "__main__":
    ...
    config_df_copy = config_df_copy.with_columns(
        pl.lit('RegressionQR').alias('model'),
        pl.lit('regression').alias('task'),
        pl.when(pl.col('strategy') == 'BackwardElimination').then(pl.lit('correlation')).otherwise(pl.lit('passthrough')).alias('ranking')
    )
    config_df_copy = config_df_copy.with_columns(
        pl.when(pl.col('task') == 'regression').then(pl.lit("{'tol': 0.001, 'metric': 'mse'}").alias('params'))
    )
    config_df = pl.concat([config_df, config_df_copy], how='vertical')
    script_dir = os.path.dirname(os.path.abspath(__file__))
    base_tables_dir = 'experiments/base_tables/'
    config_df = config_df.filter(pl.col('strategy') != 'BackwardElimination')
    for lake, config in config_df.group_by('lake'):
        ...
        for row in config.iter_rows():
            ...
            combination_hash = f'{lake}_{base_table_name}_{model}_{strategy}'
    ...
    experiment_dir = os.listdir('experiments/downstream/ablation/lda_linreg/logs/')
    trainers = {
        'simple': SimpleTrainer,
        # 'autogluon': AutoGluonTrainer
    }
    for trainer_name in trainers:
        trainer_results = pd.read_csv(f'experiments/base_tables/base_{trainer_name}_scores.csv', usecols=['Unnamed: 0', 'r2','rmse','accuracy','f1_weighted'])
        trainer_results = trainer_results.rename({'Unnamed: 0': 'table'}, axis=1)
        trainer_class = trainers[trainer_name]
        all_results = pd.DataFrame()
        for dir in experiment_dir:
            if not os.path.isdir(os.path.join('experiments/downstream/ablation/lda_linreg/logs/', dir)):
                continue
            log_path = os.path.join('experiments/downstream/ablation/lda_linreg/logs/', dir)
            print(dir)
            lake, table_name, model, strategy = dir.split('_')
            try:
                ...
                if len(augplan) == 0:
                    ...
                    df = trainer_results[trainer_results['table'] == table_name].reset_index(drop=True)
        ...
        all_results.to_csv(f'experiments/downstream/ablation/lda_linreg/logs/{trainer_name}_results.csv')