...
if __name__ == "__main__":
    ...
    config_df = config_df.filter(pl.col('strategy').is_in(['ForwardSelection']))
    script_dir = os.path.dirname(os.path.abspath(__file__))
    base_tables_dir = 'experiments/base_tables/'
    corr_choices = {
        'classification': [0.001, 0.01, 0.1, 0.3, 0.5],
        'regression': [0.1, 0.3, 0.55, 0.75, 0.9]
    }
    for corr_task in corr_choices:
        config_df_copy = config_df.filter(pl.col('task') == corr_task)
        for corr_threshold in corr_choices[corr_task]:
            for lake, config in config_df_copy.group_by('lake'):
                ...
                for row in config.iter_rows():
                    ...
                    combination_hash = f'{lake}_{base_table_name}_{corr_task}_{corr_threshold}'
                    ...
                    find_best_joins_kwargs = {'user_table_processed': X, 'top_k': 50, 'n_jobs': 16, 'config': config}
                    ...
                    if strategy != 'LassoFeatureSelector':
                        if task == 'regression':
                            find_best_joins_kwargs['corr_threshold'] = corr_threshold
    ...
    experiment_dir = os.listdir('experiments/downstream/ablation/corr/logs/')
    trainers = {
    ...
    for trainer_name in trainers:
        ...
        for dir in experiment_dir:
            if not os.path.isdir(os.path.join('experiments/downstream/ablation/corr/logs/', dir)):
                continue
            log_path = os.path.join('experiments/downstream/ablation/corr/logs/', dir)
            print(dir)
            lake, table_name, task, threshold = dir.split('_')
            try:
        ...
        all_results.to_csv(f'experiments/downstream/ablation/corr/logs/{trainer_name}_results.csv')