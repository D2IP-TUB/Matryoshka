...
if __name__ == "__main__":
    ...
    tol_choices = {
        'classification': [0.50, 0.1, 0.3, 0.5],
        'regression': [0.0001, 0.001, 0.05, 0.1]
    }
    for tol_task in tol_choices:
        task = tol_task
        config_df_copy = config_df.filter(pl.col('task') == tol_task)
        for tol_threshold in tol_choices[tol_task]:
            for lake, config in config_df_copy.group_by('lake'):
                ...
                for row in config.iter_rows():
                    ...
                    combination_hash = f'{lake}_{base_table_name}_{task}_{tol_threshold}'
                    ...
                    discovery_config_kwargs['params']['tol'] = tol_threshold
    ...
    experiment_dir = 'experiments/downstream/ablation/tol/logs/'
    ...
    for trainer_name in trainers:
        ...
        for dir in experiment_dirs:
            ...
            print(dir)
            lake, table_name, task, tol = dir.split('_')
            try:
