...
#             log_dir = os.path.join(script_dir, 'logs5', combination_hash)
#             if not os.path.exists(os.path.join(script_dir, 'logs5', combination_hash)):
#                 os.makedirs(log_dir)
...
#                 gc.collect()
    ...
    experiment_dir = os.listdir('experiments/downstream/logs5/')
    trainers = {
    ...
    for trainer_name in trainers:
        ...
        for dir in experiment_dir:
            if not (dir.endswith('autofeat') or dir.endswith('arda')):
                continue
            if not os.path.isdir(os.path.join('experiments/downstream/logs5/', dir)):
                continue
            log_path = os.path.join('experiments/downstream/logs5/', dir)
            print(dir)
        ...
        all_results.to_csv(f'experiments/downstream/logs5/{trainer_name}_results.csv')